...
class Sub_Verb_Matching:
    """
    This Class is used for check compatibility of Subject and Verb in a sentence.
    To accomplish this goal, there are sevral methods:
        1. get_subject_shakhs: Determines the "shakhs" of Subject.
        2. match_verb_with_subject: Finds a Verb that matches with the Subject.
        3. check_matching: After preprocessing, identifies Subject and Verb in the
             sentence and uses oher functions to output corrected form of the verb.
    """
    def __init__(self, vocab):
        self.vocab = vocab

        with open('resources/mokasar.txt', encoding='utf-8') as file:
            mokasar = [line.rstrip() for line in file]
        self.mokasar_verbs = mokasar

        with open('resources/verb_to_id.json', encoding='utf-8') as f:
            verb_to_id = json.load(f)
        self.verb_to_id = verb_to_id

        with open('resources/id_to_verb.json', encoding='utf-8') as f:
